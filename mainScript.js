    ...
    squares[i].addEventListener('click', function() {
        if(squares[i+7].classList.contains('taken')) {
            if(currentPlayer === 1) {
                ...
                checkTheBoard('player-two')
                currentPlayer = 1
                displayCurrentPlayer.innerHTML = currentPlayer
            }
        } else alert('not able to pick this spot')
    }) 
}
...
function checkTheBoard(player) {
    for(let j = 0; j < winningArrays.length; j++) {
        element0 = squares[winningArrays[j][0]] 
        element1 = squares[winningArrays[j][1]] 
